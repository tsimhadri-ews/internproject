...
def run_pipeline(unique_id, yaml_file):
    ...
    # Credentials read in from Github secrets for accessing Kubeflow 
    ...
    # Create a session for logging into Kubeflow 
    ...
    #Scrape the session id from hidden fields in Kubeflow for log in 
...
# Generate a unique ID so each pipeline/run created has a different name 
unique_id = datetime.now().strftime("%Y%m%d%H%M%S")

# Compile the pipeline
yaml_file = f'intrusion_pipeline_{unique_id}.yaml'
