...
def fetch_and_process_data():
    ...
    fetch_query = "SELECT md.*,mo.outcome as prediction FROM intrusion_data as md join intrusion_outcomes mo on mo.uid=md.uid WHERE md.outcome = 2 limit 1;"
    try:
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            message1 += f"UID {row['uid']} already exists in intrusion_outcomes with outcome {row['prediction']}.<br>"
            print(message1)
...
def handle_post_action(action):
    ...
    try:
        select_query = "SELECT mo.uid,mo.outcome FROM intrusion_data as md join intrusion_outcomes mo on mo.uid=md.uid WHERE md.outcome = 2 limit 1;"
